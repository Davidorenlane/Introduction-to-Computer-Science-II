...
template <typename T>
Set<T>& Set<T>::operator=(const Set<T>& n) {
	if (this != &n) { //if the two arent already at the same address
		Set<T> j(n); //copy construct a temporary Set j equivalent to n (so we can call swap (a non constant function)
...
template <typename T>
void Set<T>::swap(Set& other) { //simply sawps all the private member pointers/numitems in two sets 
	int x;
	Node* temp;
	temp = other.ptr;
	other.ptr = ptr;
...
template <typename T>
void unite(const Set<T>& s1, const Set<T>& s2, Set<T>& result) { //nonmember functions
	ItemType value1;
	ItemType value2;
